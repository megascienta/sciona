...
3. Produce an executive evaluation memo containing:
   ...
   - Tasks where SCIONA was not the right tool (and why that is expected — do not frame out-of-scope tasks as tool failures; if SCIONA was not used for a task, note whether the task was structural or semantic/behavioral, and state that the absence of usage is consistent with the tool's intended scope)
   - Aggregated scores
   - Situations where SCIONA helped
   - Situations where conventional tooling was still required (without implying SCIONA should have covered these — note the task type and why structural tooling does not apply)
