...
def call_edge_count_by_id(artifact_conn, structural_ids: Sequence[str]) -> Dict[str, int]:
    if artifact_conn is None or not structural_ids:
        return {}
    placeholders = ",".join("?" for _ in structural_ids)
    rows = artifact_conn.execute(
        f"""
        SELECT src_node_id, COUNT(*) AS edge_count
        FROM graph_edges
        WHERE edge_kind = 'CALLS'
          AND src_node_id IN ({placeholders})
        GROUP BY src_node_id
        """,
        tuple(structural_ids),
    ).fetchall()
    return {row["src_node_id"]: int(row["edge_count"]) for row in rows}
