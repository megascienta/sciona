...
def count_by_row_origin(rows: Iterable[dict[str, object]]) -> dict[str, int]:
    rows = list(rows)
    return {
        "committed_count": sum(1 for row in rows if row.get("row_origin") == "committed"),
        "overlay_added_count": sum(
            1 for row in rows if row.get("row_origin") == "overlay_added"
        ),
        "overlay_removed_count": sum(
            1 for row in rows if row.get("row_origin") == "overlay_removed"
        ),
    }
