...
"""Walker implementation for JavaScript node extraction."""
...
from .javascript_node_text import (
    function_body_node,
    name_chain,
    node_text,
)
...
def _javascript_heritage_metadata(node, content: bytes) -> dict[str, list[str]]:
    heritage_nodes = [
        child
...
def _structural_walk_node_types() -> tuple[str, ...]:
    return tuple(
        sorted(
            JAVASCRIPT_STRUCTURAL_NODE_TYPES | JAVASCRIPT_STRUCTURAL_CARRIER_NODE_TYPES
        )
    )


def walk_javascript_nodes(
    node,
    *,
    language: str,
    ...
    if node.type == "variable_declarator":
        ...
        if value_node.type == "new_expression":
            if name_node.type == "identifier":
                callee = value_node.child_by_field_name(
                    "constructor"
                ) or value_node.child_by_field_name("function")
    ...
    if node.type == "field_definition" and state.class_stack:
        name_node = node.child_by_field_name("name")
...
__all__ = ["walk_javascript_children", "walk_javascript_nodes"]
