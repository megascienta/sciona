    ...
    if not no_interactive and sys.stdin.isatty():
        ...
        if agents_path.exists():
            ...
            if action == "overwrite":
                ...
                return
        path = cli_call(
            repo_ops.init_agents, repo_root, mode=mode, commands=agents_command_map()
        )
...
def _maybe_init_hook(
    ...
    *,
    ...
    if no_interactive:
        if install:
            cli_call(repo_ops.install_commit_hook, cmd, repo_root)
        return
    ...
    cli_call(repo_ops.install_commit_hook, cmd_input.strip() or cmd, repo_root)
