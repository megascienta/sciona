...
- `structural_accuracy`: Extracts 40-hex ID pairs from reducer output and compares each to known edges in SCIONA DB or artifacts DB. Accuracy = 1 - (invalid_pairs / total_pairs). Defaults to 1.0 when no pairs are found.
- `identifier_overlap` (formerly `semantic_fidelity`): For each invocation, extracts identifier terms from direct code (tree-sitter preferred, AST fallback). For structured reducers, compares to identifier tokens found in contract evidence/required fields (echo and content-hash fields excluded). Overlap = matched_terms / total_terms. Source reducers match terms in raw output text. Summary reducers are excluded from overlap aggregation because they do not emit many identifiers.
...
- `schema_compliance_score`: Proportion of required fields/paths, type checks, and invariants satisfied (as defined by reducer contracts).
...
- `content_hash_match_rate`: Fraction of content hashes that match the extracted snippet (with normalization). Mismatches can reflect normalization differences rather than structural errors. Hash diagnostics (span, snippet preview, candidate hashes) are always collected.
