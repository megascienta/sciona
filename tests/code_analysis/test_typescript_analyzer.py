...
def test_typescript_analyzer_sets_decorated_start_line_for_decorated_members(tmp_path):
    module = """@Component
export class Widget {
  @Input()
  value() {
...
"""
    ...
    assert value.decorated_start_line == value.start_line - 1
...
def test_typescript_analyzer_rejects_relative_imports_escaping_repo_root(tmp_path):
    repo = tmp_path
    src = repo / "src"
    src.mkdir()
    file_path = src / "mod.ts"
    module = "import { helper } from '../../../secret';\n"
    file_path.write_text(module, encoding="utf-8")
    snapshot = FileSnapshot(
        record=FileRecord(
            path=file_path,
...
def test_typescript_analyzer_emits_kind_metadata_for_interface_and_signatures(tmp_path):
    ...
    assert (worker_node.metadata or {}).get("extends_bases") == ["Base"]
    assert (worker_node.metadata or {}).get("implements_bases") == []


def test_typescript_analyzer_preserves_extends_and_implements_edge_semantics(tmp_path):
    module = """
    export class Base {}
    ...
    edge_types = {
        (edge.dst_qualified_name, edge.edge_type)
        for edge in result.edges
        if edge.src_qualified_name == f"{module_name}.Worker"
    }
    assert (f"{module_name}.Base", "EXTENDS") in edge_types
    assert (f"{module_name}.Runnable", "IMPLEMENTS") in edge_types
